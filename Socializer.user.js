...
angular.module('Socializer', []).controller('MainCtrl', ['$scope', '$http', '$interval','$compile', function($scope, $http, $interval, $compile) {
    ...
    $scope.reset = function(){
        if (confirm('This will reset your settings')){
            $scope.data = {
                pools: [],
                blogs: [],
                me: currentLogin.email,
                root: BlogsBaseUrl // "/blogs"
            };
            localStorage['Socializer'] = JSON.stringify($scope.data);
	...
	$scope.global = {
		modes: [
			{value: 0, name: 'Inactive', icon: 'not_interested'},
			{value: 1, name: 'Only notify', icon: 'sms'},
			{value: 2, name: 'Wait for approval', icon: 'play_circle_outline'},
			{value: 3, name: 'Fully automated', icon: 'autorenew'}
		]
	};
	$scope.notifications = [];
    $scope.posts = [];
	...
	$scope.checkBlog = function(blog){
        ...
        $scope.notifications.push({blog: blog, date: new Date(Date.now()), html:'Checking "'+blog.name+'"...'});
		$.ajax({
	...
	$scope.comment = function(entry, blog){
        ...
        if (blog.mode == 3){
            ...
            $scope.notifications.push({blog: blog, entry: entry, date: new Date(Date.now()), html:'"' + response + '" auto published to <a href="' + entry["_xml:base"] + '" target="_BLANK">' + entry.title[0].__text + '</a>.'});
